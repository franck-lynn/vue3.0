...
const config = {
    mode: 'development',
    entry: "./test/main.js", // webpack 打包的入口文件
    output: { filename: "bundle.js", path: path.join(__dirname, 'dist') }, // 出口文件ename: 'vuer-ui.js'
    // },
    module: {
        rules: [
            ...
            // { test: /\.scss$/, use: ["style-loader", 'css-loader', { loader: 'sass-loader', options: { implementation: require("sass"), sassOptions: { fiber: false } } }]},
            {
                ...
                use: [
                    ...
                    //         resources: ['./packages/scss/main.scss']
                    //     }
                    // }
                ]
            },
    ...
    // exclude: ['./assets/', './public/', './vuer-ui组件库练习实例.md', './正式上传npm.md'],
    resolve: {
        alias: {
            // 路径的别名
            '@': path.resolve('src'),
