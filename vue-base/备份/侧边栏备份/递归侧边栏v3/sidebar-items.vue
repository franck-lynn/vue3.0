<template>
    <ul :class="[isDeepOne ? 'nav-list' : 'sub-list']">
        ...
        <li v-for="(item, index) in items" :key="index" :class="[isDeepOne? 'nav-items': 'nav-sub-items', 
          `nav-list__level-${deep}`, 
          {'pulldown-open': currentTarget? currentTarget.title === item.title: false,
          'pulldown-show': currentTarget.title === item.title ? isShow: ''}]" @click="handleCurrentTarget($event, deep)">
